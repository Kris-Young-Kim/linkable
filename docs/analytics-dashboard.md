...
- `components/analytics/trend-charts.tsx`: 시간별 트렌드 차트 컴포넌트 (일별/주별/월별, 시간대별)
...
- **일별 트렌드**: 최근 30일간의 일별 추천 생성 수, K-IPPA 평가 수, 상담 수
- **주별 트렌드**: 최근 12주간의 주별 활동 통계
- **월별 트렌드**: 최근 12개월간의 월별 활동 통계 및 신규 사용자 수 (관리자용)
- **시간대별 활동 패턴**: 24시간 기준 시간대별 상담 시작 수 및 추천 클릭 수
- **시각화**: 라인 차트 (일별), 바 차트 (주별/월별/시간대별)
- **API**: `GET /api/admin/analytics`, `GET /api/analytics` (trends 필드에 포함)
