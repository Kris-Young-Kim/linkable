...
2. [ ] **상품 크롤링 및 자동 등록 시스템 구현**
   ...
   - [x] **웹 스크래핑 크롤러 구현** (기본 구조 완료) ✅
     ...
     - [x] 관리자 UI에 크롤링 기능 추가 (`/admin/products`) ✅
       - [x] 크롤링 실행 버튼 (키워드/ISO 코드 입력) ✅
       - [x] 크롤링 결과 미리보기 및 선택적 등록 ✅
       - [x] 크롤링 로그 및 에러 처리 ✅
...
5. [x] 채팅/추천/대시보드 CTA 버튼 통일 (컴포넌트화)
6. [x] 접근성 체크리스트 (포커스 링, SR 텍스트, 대비) 실행
7. [x] 상담→추천 플로우 안내 모달/토스트 설계
...
11. [x] 추천 카드 CTA: 구매하기/지원제도/전문가 문의 버튼 추가
12. [x] 포인트/쿠폰 인센티브 로직 설계 + DB 필드 추가
13. [x] 클릭/전환 이벤트 로깅 + Analytics 대시보드와 연동
