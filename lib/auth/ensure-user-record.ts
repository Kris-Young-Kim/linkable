...
export async function ensureUserRecord(clerkUserId: string): Promise<string> {
  ...
  if (existingByEmail?.id) {
    ...
    if (existingByEmail.clerk_id !== clerkUserId) {
      ...
      if (updateError) {
        console.error("[ensureUserRecord] Failed to update clerk_id:", updateError)
  ...
  if (insertError) {
    ...
    if (insertError.code === "23505" && insertError.message?.includes("email")) {
      console.warn("[ensureUserRecord] Email duplicate detected, fetching existing user:", email)
      ...
      if (existingUser?.id) {
        ...
        if (existingUser.clerk_id !== clerkUserId) {
          ...
          if (updateError) {
            console.error("[ensureUserRecord] Failed to update clerk_id after duplicate:", updateError)
          } else {
            logEvent({
              category: "system",
              action: "user_clerk_id_updated",
