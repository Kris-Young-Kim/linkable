...
export const iso9999Catalog: IsoCodeInfo[] = [
  ...
  },
  {
    ...
    label: "읽기 보조기기",
    description:
      "시각 장애인을 위한 점자 디스플레이, 스크린 리더, 음성 변환 기기",
    class: "22",
    subclass: "2206",
  },
  {
...
/**
 * ISO 코드로 정보 조회 (동기, static catalog 사용)
 *
 * @deprecated Full ISO 코드를 데이터베이스에서 조회하므로 getIsoCodeInfoAsync를 사용하세요.
 *             하위 호환성을 위해 유지됩니다.
 */
export function getIsoCodeInfo(iso: string): IsoCodeInfo | undefined {
  return iso9999Catalog.find(
    (item) =>
      item.iso === iso || item.iso.replace(/\s/g, "") === iso.replace(/\s/g, "")
  );
}
...
export async function getIsoCodeInfoAsync(
  ...
  if (supabase) {
    try {
      ...
      const isoWithSpace = iso.includes(" ")
        ? iso
        : `${iso.slice(0, 2)} ${iso.slice(2)}`;
      ...
      console.error(
        `[getIsoCodeInfoAsync] Database lookup failed for ${iso}:`,
        error
      );
...
/**
 * 클래스별 ISO 코드 목록 조회 (동기, static catalog 사용)
 *
 * @deprecated Full ISO 코드를 데이터베이스에서 조회하므로 getIsoCodesByClassAsync를 사용하세요.
 *             하위 호환성을 위해 유지됩니다.
 */
...
export async function getIsoCodesByClassAsync(
  ...
  if (supabase) {
    try {
      ...
      console.error(
        `[getIsoCodesByClassAsync] Database lookup failed for class ${classCode}:`,
        error
      );
      // 데이터베이스 조회 실패 시 fallback
    }
...
/**
 * 검색어로 ISO 코드 검색 (동기, static catalog 사용)
 *
 * @deprecated Full ISO 코드를 데이터베이스에서 조회하므로 searchIsoCodesAsync를 사용하세요.
 *             하위 호환성을 위해 유지됩니다.
 */
  ...
  if (supabase) {
    try {
      const { data: isoCodes } = await supabase
        ...
        .select("code, name, description, parent_code, level")
        .or(
          `code.ilike.%${normalizedQuery}%,name.ilike.%${normalizedQuery}%,description.ilike.%${normalizedQuery}%`
        )
        .eq("is_active", true)
        .order("code", { ascending: true });
      ...
      console.error(
        `[searchIsoCodesAsync] Database lookup failed for query "${query}":`,
        error
      );
      // 데이터베이스 조회 실패 시 fallback
    }
...
/**
 * 모든 ISO 코드 목록 반환 (정렬됨, 동기, static catalog 사용)
 *
 * @deprecated Full ISO 코드를 데이터베이스에서 조회하므로 getAllIsoCodesAsync를 사용하세요.
 *             하위 호환성을 위해 유지됩니다.
 */
