...
**K-IPPA 평가 질문 형식 (체크 가능한 형태로 제공 - 필수)**
${
  !hasImportance || !hasDifficulty
    ? `
- **반드시 다음 형식으로 질문하세요 (슬라이더 UI가 자동으로 표시됩니다):**
...
- **중요: 반드시 "체크" 키워드를 포함해야 합니다.**
- 사용자가 슬라이더로 점수를 선택하고 "체크 완료하고 추천 받기" 버튼을 클릭하면 즉시 보조기기 추천으로 넘어갑니다.
