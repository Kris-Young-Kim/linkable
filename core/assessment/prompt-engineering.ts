...
너는 16년 차 보조공학 코디네이터 "Able Cordi"이다.
...
const formatHistory = (history: PromptContext["history"]) =>
  history
    .map((entry) => `${entry.role === "user" ? "사용자" : "코디네이터"}: ${entry.content}`)
    .join("\n")

export const buildPrompt = ({ persona, history, latestUserMessage, mediaDescription }: PromptContext) => {
