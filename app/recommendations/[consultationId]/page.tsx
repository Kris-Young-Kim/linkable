...
const RecommendationsViewWithFilters = dynamic(
  () =>
    import(
      "@/components/recommendations/recommendations-view-with-filters"
    ).then((mod) => ({ default: mod.RecommendationsViewWithFilters })),
  {
    loading: () => (
...
const ConsultationFeedbackForm = dynamic(
  () =>
    import("@/components/consultation-feedback-form").then((mod) => ({
      default: mod.ConsultationFeedbackForm,
    })),
  {
    loading: () => (
      <Card>
...
async function fetchConsultationData(
  ...
  try {
    ...
    console.warn(
      "[recommendations page] ippa_activities 조회 실패 (무시됨):",
      error
    );
  }
...
async function fetchRecommendations(consultationId: string) {
  ...
  const response = await fetch(
    `${baseUrl}/api/products?consultationId=${consultationId}`,
    {
      cache: "no-store",
...
export default async function RecommendationsDetailPage({
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development" && (data as any)._debug) {
      console.log(
        "[recommendations page] API 응답 디버깅 정보:",
        (data as any)._debug
      );
    }
    ...
    if (products.length === 0) {
      console.warn(
        "[recommendations page] 제품이 없습니다. ICF 코드:",
        consultationData.analysis?.icfCodes
      );
    }
  } catch (error) {
        ...
        <div className="container mx-auto px-4 md:px-6 py-4">
          <Breadcrumbs
            ...
            items={[
              ...
              {
                translationKey: "breadcrumbs.recommendations",
          ...
          <CardHeader className="pb-0">
            <LocalNav
              items={[
                {
                  label: "전체 추천",
                  href: `/recommendations/${consultationId}?filter=${filterBy}&sort=${sortBy}`,
                },
                {
                  label: "필터",
                  href: `/recommendations/${consultationId}?filter=favorites&sort=${sortBy}`,
                  badge: "Soon",
                },
                {
                  label: "평가 대기",
                  href: `/recommendations/${consultationId}?filter=pending&sort=${sortBy}`,
                  badge: "Soon",
                },
              ]}
