...
const BaselineEvaluationClient = dynamic(
  () =>
    import("@/components/ippa/baseline-evaluation-client").then((mod) => ({
      default: mod.BaselineEvaluationClient,
    })),
  {
    loading: () => (
      <Card>
...
export default async function BaselineEvaluationPage({
  ...
  if (!userId) {
    redirect(
      `/sign-in?redirect_url=/dashboard/ippa/baseline/${consultationId}`
    );
  }
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                보조기기 사용 전 현재 상태를 평가해주시면, 사용 후 개선도를
                정확히 측정할 수 있습니다.
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                AI가 상담 내용에서 추출한 활동 항목에 대해 중요도와 현재 어려움
                정도를 평가해주세요.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <BaselineEvaluationClient
                ...
                problemDescription={
                  analysis?.identifiedProblems || analysis?.summary || ""
                }
              />
            </CardContent>
          </Card>
