...
export async function GET(request: NextRequest) {
  try {
    ...
    const status = (searchParams.get("status") as "APPROVED" | "CANCELED") || "APPROVED"

    console.log(`[Purchase Report] 구매 리포트 조회 시작: ${startDate} ~ ${endDate}`)

    const coupangClient = createCoupangClient()
    if (!coupangClient) {
      return NextResponse.json(
        { error: "쿠팡 API 클라이언트를 생성할 수 없습니다. 환경 변수를 확인하세요." },
        { status: 500 }
      )
    }

    // 쿠팡 API로 구매 리포트 조회
    const purchases = await coupangClient.getPurchaseReport(startDate, endDate, status)
