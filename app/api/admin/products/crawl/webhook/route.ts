    ...
    for (const p of products) {
      try {
        ...
        if (!isoCode || isoCode === "N999999" || isoCode === "00 00") {
          try {
            const { inferIsoCodeFromProduct } = await import(
              "@/core/matching/ai-iso-inference"
            );
            ...
            if (aiResult && aiResult.confidence >= 0.5) {
              ...
              console.log(
                `[Crawler Webhook] Auto-inferred ISO code for "${p.title}": ${isoCode} (confidence: ${aiResult.confidence})`
              );
            } else {
              // AI 추론 실패 시 키워드 기반 추론 시도
              const { searchIsoCodesAsync } = await import(
                "@/lib/iso-9999-catalog-async"
              );
              const searchResults = await searchIsoCodesAsync(
                p.title,
                supabase
              );
              if (searchResults.length > 0) {
                ...
                console.log(
                  `[Crawler Webhook] Keyword-matched ISO code for "${p.title}": ${isoCode}`
                );
              }
            }
          } catch (error) {
            console.warn(
              `[Crawler Webhook] ISO inference failed for "${p.title}":`,
              error
            );
          }
        }
        ...
        if (isoCode && isoCode !== "N999999" && isoCode !== "00 00") {
          const { convertToDivisionLevel } = await import(
            "@/lib/utils/iso-code-converter"
          );
          const convertedCode = await convertToDivisionLevel(isoCode, supabase);
          if (convertedCode) {
            isoCode = convertedCode;
            console.log(
              `[Crawler Webhook] Converted to Division level: ${isoCode}`
            );
          }
        } else {
          // 추론 실패 시 기본값
        ...
        if (!isoCodeId) {
          console.warn(
            `[Crawler Webhook] Skipping product "${p.title}": iso_code_id is null (ISO: ${isoCode})`
          );
          results.failed++;
          results.errors.push({
