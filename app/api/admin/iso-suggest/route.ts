...
export async function POST(request: Request) {
  ...
  const isoKeywordMap: Record<string, string[]> = {
    "15 09": ["식기", "식사", "숟가락", "포크", "컵", "음주", "무게조절", "적응형", "식사도구", "식사보조", "식사기구"],
    "12 22": ["휠체어", "휠체", "수동", "wheelchair", "의자형", "바퀴"],
    "12 23": ["휠체어", "전동", "전기", "모터", "전동휠체어", "전동의자", "전동체어"],
    "12 06": ["보행기", "워커", "지팡이", "목발", "보행보조", "보행도구", "보행장비"],
    "12 03": ["지팡이", "목발", "한팔", "보행"],
    "12 08": ["안내", "지팡이", "시각", "맹인"],
    "12 31": ["체위", "리프트", "앉기", "서기", "전동의자", "리프트체어", "기립"],
    "18 30": ["경사로", "승강기", "램프", "접근성", "문턱", "계단", "수직"],
    "18 18": ["손잡이", "그랩바", "핸드레일", "안전바", "지지대", "레일"],
    "09 33": ["목욕", "샤워", "세면", "욕조", "욕실", "의자", "욕실의자", "샤워의자"],
    "09 18": ["옷", "착의", "의복", "입기", "착용"],
  ...
  for (const isoInfo of iso9999Catalog) {
    ...
    if (productNameLower.includes(isoLower.replace(/\s/g, "")) || productNameLower.includes(isoLower)) {
  ...
  try {
    ...
    if (aiResult) {
      ...
      const isoInfo = iso9999Catalog.find(
        (item) => item.iso === aiResult.isoCode || item.iso.replace(/\s/g, "") === aiResult.isoCode.replace(/\s/g, "")
      )
      ...
      if (aiResult.alternativeCodes && aiResult.alternativeCodes.length > 0) {
        for (const altCode of aiResult.alternativeCodes) {
          const altIsoInfo = iso9999Catalog.find(
            (item) => item.iso === altCode.isoCode || item.iso.replace(/\s/g, "") === altCode.isoCode.replace(/\s/g, "")
          )

          if (altIsoInfo) {
            const existing = keywordMatches.find((m) => m.iso === altIsoInfo.iso)
            if (existing) {
              existing.score += altCode.confidence * 1.5
            } else {
              const existingAi = aiMatches.find((m) => m.iso === altIsoInfo.iso)
              if (!existingAi) {
                aiMatches.push({
                  iso: altIsoInfo.iso,
  ...
  // 상위 5개만 반환
  const suggestions = sortedMatches.slice(0, 5).map(({ matchedKeywords, ...rest }) => rest)
