...
function parseArgs(): CliOptions {
  ...
  if (
    ...
    throw new Error(
      "--keyword, --category, 또는 --categories 옵션이 필요합니다. 또는 --list-sites로 지원 사이트 목록을 확인하세요."
    );
  }
  ...
  for (const product of products) {
    try {
      ...
      if (existing) {
        ...
        result.updated++;
        console.log(
          `  🔄 업데이트: ${product.name} (ISO: ${isoCode}${isoCode !== product.iso_code ? `, 변환됨: ${product.iso_code} → ${isoCode}` : ''})`
        );
      } else {
        // 신규 생성
        const { data, error } = await supabase
...
async function main() {
  try {
    ...
    if (options.listSites) {
      ...
      console.log("\n사용법:");
      console.log(
        '  pnpm crawl:products --keyword "검색어" --platform ablelife'
      );
    ...
    if (options.isoCode && !isValidIsoCodeFormat(options.isoCode)) {
      console.warn(`⚠️  ISO 코드 형식이 올바르지 않습니다: ${options.isoCode}`);
      console.warn("올바른 형식: 'XX XX' (예: '15 09')");
      console.warn(
        "계속 진행하지만 상품은 추천에 사용되지 않을 수 있습니다.\n"
      );
    }
          ...
          for (const site of sites) {
            ...
            try {
              ...
              if (result.success && result.products.length > 0) {
                console.log(
                  `✅ ${site.name} (${category}): ${result.products.length}개 상품 수집`
                );
                allProducts.push(
                ...
                console.warn(
                  `⚠️  ${site.name} (${category}): 수집된 상품이 없습니다.`
                );
              }
              if (result.errors && result.errors.length > 0) {
                console.warn(
                  `⚠️  ${site.name} (${category}) 에러: ${result.errors.join(
                    ", "
                  )}`
                );
              }
    ...
    if (
      ...
      for (const site of sites) {
        ...
        try {
          ...
          if (result.success && result.products.length > 0) {
            console.log(
              `✅ ${site.name}: ${result.products.length}개 상품 수집`
            );
            allProducts.push(
              ...result.products.map((p) => ({
                ...p,
    ...
    if (testError) {
      ...
      console.error(
        `   Service Role Key: ${serviceRoleKey ? "설정됨" : "설정되지 않음"}`
      );
    ...
    if (result.created > 0 || result.updated > 0) {
      ...
      console.log(
        `✅ 데이터베이스에 활성 제품 ${savedCount || 0}개가 저장되어 있습니다.`
      );
    }
  } catch (err) {
