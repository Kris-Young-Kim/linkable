...
// 다른 모듈을 import하기 전에 먼저 환경 변수를 로드해야 함
...
// 환경 변수 로드 후에 다른 모듈은 함수 내부에서 동적 import
...
function parseIcfCatalog(content: string): IcfCodeEntry[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("## B. 신체 기능") || line.includes("B. 신체 기능")) {
      ...
      line.startsWith("## S. 신체 구조") ||
      line.includes("S. 신체 구조")
    ) {
      currentCategory = "s";
      continue;
    } else if (
      line.startsWith("## D. 활동과 참여") ||
      line.includes("D. 활동과 참여")
    ) {
      currentCategory = "d";
      continue;
    } else if (
      line.startsWith("## E. 환경 요인") ||
      line.includes("E. 환경 요인")
    ) {
      currentCategory = "e";
      continue;
    ...
    const codeMatch = line.match(
      /^###\s+([BDSESP]\d+[A-Z0-9]*):\s+([a-z]\d+[a-z0-9]*)\s*(.*)$/i
    );
...
async function syncIcfCatalog() {
  try {
    ...
    for (let i = 0; i < codes.length; i += batchSize) {
      ...
      if (error) {
        console.error(
          `[ICF Catalog Sync] 배치 ${i / batchSize + 1} 오류:`,
          error
        );
        continue;
      }
      ...
      const newCount =
        data?.filter((d) => d.created_at === d.updated_at).length || 0;
