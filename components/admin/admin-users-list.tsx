...
export function AdminUsersList() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      setError(null)
      try {
        console.log("[Admin Users List] Fetching users...")
        ...
        setUsers(data.users || [])
        console.log(`[Admin Users List] Fetched ${data.users?.length ?? 0} users`)
      } catch (err) {
        console.error("[Admin Users List] Fetch error:", err)
