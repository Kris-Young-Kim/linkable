...
interface IppaConsultationFormProps {
  ...
  onComplete: (data: { activities: ActivityScore[] }) => void | Promise<void>;
  onSkip?: () => void;
...
export function IppaConsultationForm({
      ...
      fetch(`/api/consultations/${consultationId}/icf-codes`)
        ...
        .then((data) => {
          if (data.icfCodes && Array.isArray(data.icfCodes)) {
            ...
            const dCodes = data.icfCodes.filter(
              (code: string) =>
  ...
  const handleScoreChange = (
    code: string,
    field: "importance" | "currentDifficulty",
    value: number[]
  ) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onComplete({ activities });
    } catch (error) {
      console.error("[K-IPPA Consultation Form] 제출 오류:", error);
      ...
      <CardContent className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="rounded-lg border border-yellow-200 bg-yellow-50 dark:bg-yellow-950/20 p-4">
            <p className="text-sm text-yellow-900 dark:text-yellow-100">
              상담에서 추출된 ICF 활동 코드가 없습니다. 상담을 먼저
              진행해주세요.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-3">
                AI가 상담 내용을 분석하여 추출한 {extractedIcfCodes.length}개의
                활동에 대해 중요도와 어려운 정도를 평가해주세요.
              </p>
            </div>
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
              {extractedIcfCodes.map((code) => {
                ...
                const scores = activityScores[code] || {
                  importance: 3,
                  currentDifficulty: 3,
                };
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Slider
                          ...
                          onValueChange={(v) =>
                            handleScoreChange(code, "importance", v)
                          }
                          className="w-full"
                        />
                      ...
                      <div className="space-y-2">
                        ...
                        <Slider
                          ...
                          onValueChange={(v) =>
                            handleScoreChange(code, "currentDifficulty", v)
                          }
                          className="w-full"
                        />
                        <p className="text-xs text-muted-foreground">
                      ...
                      <div className="rounded-lg border border-primary/20 bg-primary/5 p-2">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-semibold">
                            {scores.importance} × {scores.currentDifficulty} ={" "}
                            {scores.importance * scores.currentDifficulty}
                          </span>
                        </div>
                      </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {isSubmitting
              ? "제출 중..."
              : `제출 (${extractedIcfCodes.length}개 활동)`}
          </Button>
          {onSkip && (
            <Button
              onClick={onSkip}
